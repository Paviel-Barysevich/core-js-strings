 ...
 */
function concatenateStrings(value, ...args) {
...
function removeFirstOccurrences(str, value) {
  const firstIndx = str.indexOf(value);
  const lastIndx = firstIndx + value.length;

  if (firstIndx === -1) return str;

  return `${str.substring(0, firstIndx)}${str.substring(lastIndx)}`;
}
...
function removeLastOccurrences(str, value) {
  const firstIndx = str.lastIndexOf(value);
  const lastIndx = firstIndx + value.length;
...
function sumOfCodes(str) {
  ...
  return str.split('').reduce((sum, symbol) => sum + symbol.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  const sameCaseStr = str.toLowerCase();
  let count = 0;

  for (let i = 0; i < sameCaseStr.length; i += 1) {
    if (vowelString.includes(sameCaseStr[i])) {
      count += 1;
    }
  }
...
function isPalindrome(str) {
  const onlyLettersStr = str.toLowerCase().replace(/[^a-zа-я]/g, '');
  const reversedOnlyLettersStr = onlyLettersStr.split('').reverse().join('');

  return onlyLettersStr === reversedOnlyLettersStr;
}
